...
//        }
        ...
        if($line_rec_id!=null){
            for($i=0;$i<=count($line_rec_id)-1;$i++){
                ...
                if($model_trans->save(false)){
                    $model_stock = \backend\models\Stocksum::find()->where(['item_id'=>$line_product_id[$i],'warehouse_id'=>1])->one();
                    if($model_stock){
    ...
    public function getOnhand($product_id){
        ...
        if($product_id){
            $onhand = \backend\models\Stocksum::find()->where(['item_id'=>$product_id,'warehouse_id'=>1])->sum('qty');
